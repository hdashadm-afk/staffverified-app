...
| Payslip email delivery | 🟡 | **Send Payslips** button on a payroll run emails each employee's payslip summary to the address on their profile and marks the run completed (PR #21). Code is merged, but **pending owner setup — not yet verified against a real inbox**: needs `GMAIL_USER`/`GMAIL_APP_PASSWORD` (Gmail app password for `heliumhr01@gmail.com`) added to Vercel env vars on both projects, then a redeploy. Also depends on employees actually having an email on file (migration 019, employee Email field, PR #20). |
