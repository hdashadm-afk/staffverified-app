...
export default function DOEPumpPriceForm({
  ...
  if (preview) {
    return (
      <div>
        <button onClick={() => setPreview(false)} className="mb-4 text-sm text-blue-600 hover:underline no-print">
          ← Back to form
        </button>
                ...
                {stations.map(s => (
                  <td key={s.id} className="px-4 py-2 text-center">
                    <input
                      ...
                      className="w-24 border border-gray-200 rounded px-2 py-1.5 text-sm text-center focus:outline-none focus:ring-1 focus:ring-blue-500"
                    />
                  </td>
                ))}
              </tr>
